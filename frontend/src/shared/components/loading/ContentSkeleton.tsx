...
import { Box, Card, CardContent, Skeleton, List, ListItem } from '@mui/material'
    ...
    <List>
      <ListItem>
        <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
          ...
          <Skeleton variant="rectangular" width={60} height={24} animation={animation} sx={{ borderRadius: 1 }} />
        </Box>
      </ListItem>
    </List>
  )
    ...
    <Box sx={{ p: 2 }}>
      ...
      {[1, 2, 3].map((field) => (
        <Box key={field} sx={{ mb: 3 }}>
          ...
          <Skeleton variant="rectangular" width="100%" height={56} animation={animation} sx={{ borderRadius: 1 }} />
        </Box>
      ))}
      
      <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', mt: 4 }}>
        <Skeleton variant="rectangular" width={80} height={40} animation={animation} sx={{ borderRadius: 1 }} />
        <Skeleton variant="rectangular" width={100} height={40} animation={animation} sx={{ borderRadius: 1 }} />
      </Box>
    </Box>
  )
  ...
  const renderChatSkeleton = () => (
    <Box sx={{ p: 2 }}>
      {[1, 2, 3].map((message) => (
        ...
        >
          {message % 2 === 1 && (
            <Skeleton variant="circular" width={32} height={32} animation={animation} sx={{ mr: 2 }} />
          )}
          <Box sx={{ maxWidth: '70%' }}>
            <Skeleton
              variant="rectangular"
              ...
              sx={{ borderRadius: 2 }}
            />
            <Skeleton variant="text" width="40%" height={12} animation={animation} sx={{ mt: 0.5 }} />
          </Box>
          {message % 2 === 0 && (
            <Skeleton variant="circular" width={32} height={32} animation={animation} sx={{ ml: 2 }} />
          )}
        </Box>
      ))}
    </Box>
