...
const MobileTableCard = <T extends Record<string, unknown>>({row, columns, onRowClick}: {
  ...
  const renderCellValue = (column: TableColumn<T>, value: unknown): React.ReactNode => {
  ...
  const getValue = (column: TableColumn<T>): unknown => {
                ...
                <Collapse in={expanded} timeout="auto" unmountOnExit>
                  <Box sx={{mt: 2, pt: 2, borderTop: 1, borderColor: 'divider'}}>
                    {secondaryColumns.map((column, index) => {
                      ...
                      return (
                          <Box key={column.id}
                               sx={{mb: index < secondaryColumns.length - 1 ? 1 : 0}}>
                            <Typography variant="caption" color="text.secondary" display="block">
                              {column.label}
                            </Typography>
...
export const ResponsiveTable = <T extends Record<string, unknown>>({
                                                                     ...
                                                                     onRowClick,
                                                                     onSort,
                                                                     getRowId = (row) => (row as Record<string, unknown> & {
                                                                       id?: string
  ...
  // Handle sorting
  const handleSort = (columnId: string): boolean => {
    if (!sortable) return
  ...
  }>[] = [
    {
      ...
      renderCell: (value, row) => (
          <Box sx={{display: 'flex', alignItems: 'center', gap: 2}}>
            <Avatar src={row.avatar as string} alt={row.name as string}
