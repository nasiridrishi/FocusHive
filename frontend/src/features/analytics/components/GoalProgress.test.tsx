            ...
            <div
              ...
              onKeyDown={e => {
                if (e.key === 'Enter' || e.key === ' ') {
                  mockFilterCategory = goal.category;
                }
              }}
            >
              <h6>{goal.title}</h6>
              ...
              {goal.deadline && <div>Due: {goal.deadline.toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
              })}</div>}
              {goal.deadline && <div>16 days left</div>}
              <div data-testid={`progress-bar-${goal.id}`}
                   aria-valuenow={Math.min(Math.round(goal.progress * 100), 100)}></div>
              {goal.progress >= 1 && <span>Completed!</span>}
              {goal.deadline && new Date() > goal.deadline && goal.progress < 1 &&
                <span>Overdue</span>}
...
const mockGoals: GoalProgressData[] = [
  {
    ...
    milestones: [
      {value: 60, label: '1 hour', achieved: true, achievedAt: new Date('2024-01-01T09:00:00Z')},
      {
        ...
        label: '2 hours',
        achieved: true,
        achievedAt: new Date('2024-01-01T11:00:00Z')
      },
      {
        value: 180,
        label: '3 hours',
        achieved: true,
        achievedAt: new Date('2024-01-01T14:00:00Z')
      },
    ...
    milestones: [
      {
        ...
        label: '2.5 hours',
        achieved: true,
        achievedAt: new Date('2024-01-01T12:00:00Z')
      },
      {
        value: 300,
        label: '5 hours',
        achieved: true,
        achievedAt: new Date('2024-01-02T16:00:00Z')
      },
      {
        value: 450,
        label: '7.5 hours',
        achieved: true,
        achievedAt: new Date('2024-01-03T14:00:00Z')
      },
      {
        value: 600,
        label: '10 hours',
        achieved: true,
        achievedAt: new Date('2024-01-04T11:00:00Z')
      }
    ]
  },
  {
    ...
    milestones: [
      {
        ...
        achieved: true,
        achievedAt: new Date('2024-01-02T18:00:00Z')
      },
