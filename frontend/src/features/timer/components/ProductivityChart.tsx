...
export const ProductivityChart: React.FC<ProductivityChartProps> = ({
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalSessions = data.reduce((sum, item) => {
      return sum + ('sessions' in item ? item.sessions || 0 : 0)
    }, 0)
    const avgProductivity = data.reduce((sum, item) => {
      return sum + ('productivity' in item ? item.productivity || 0 : 0)
    }, 0) / data.length
    const totalGoals = data.reduce((sum, item) => {
      return sum + ('goals' in item ? item.goals || 0 : 0)
    }, 0)
  ...
  const renderChart = () => {
    ...
    const sessionsData = data.map(item => 'sessions' in item ? item.sessions : 0)
    const productivityData = data.map(item => 'productivity' in item ? item.productivity : 0)
