...
const ForumPostList: React.FC = () => {
  ...
  const handleMenuClick = (event: React.MouseEvent<HTMLButtonElement>, post: ForumPost) => {
  ...
  const getPostStatusIcons = (post: ForumPost) => {
    const icons = []
                      ...
                      post.lastReply && (
                        <Box mt={1}>
                          <Typography variant="caption" color="textSecondary">
                            Last reply by <strong>{post.lastReply.author.username}</strong> • {formatTimeAgo(post.lastReply.createdAt)}
                          </Typography>
                        </Box>
                      )
                    }
