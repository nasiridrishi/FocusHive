...
import {BuddyCheckin, BuddyRelationship} from '../types'
...
const BuddyCheckinDialog: React.FC<BuddyCheckinDialogProps> = ({
  ...
  const handleMoodRatingChange = (_event: React.SyntheticEvent, value: number | null): void => {
  ...
  const handleProgressRatingChange = (_event: React.SyntheticEvent, value: number | null): void => {
  ...
  const getMoodLabel = (rating: number | null): string => {
    if (!rating) return ''
    const mood = moodEmojis.find(m => m.value === rating)
    return mood ? `${mood.emoji} ${mood.label}` : ''
  }

  const getProgressLabel = (rating: number | null): string => {
    if (!rating) return ''
                  ...
                  <CardContent>
                    <Box display="flex" alignItems="center" gap={2}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" color="textSecondary">
                          Partner since {new Date(relationship.startDate || new Date()).toLocaleDateString()}
                        </Typography>
                      </Box>
