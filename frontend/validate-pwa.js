...
function extractManifestFromViteConfig() {
  ...
  const manifestMatch = viteConfig.match(/manifest:\s*{([\s\S]*?)},?\s*devOptions/);
  if (!manifestMatch) {
...
function validatePWAManifest() {
  ...
  try {
    ...
    // Icons validation
    const hasRequiredIcons = manifestContent.includes('192x192') && manifestContent.includes('512x512');
    console.log(`  ${hasRequiredIcons ? '✅' : '❌'} Required icon sizes (192x192, 512x512)`);
    
    // Display mode
    const hasStandaloneDisplay = manifestContent.includes("display: 'standalone'");
    console.log(`  ${hasStandaloneDisplay ? '✅' : '❌'} Standalone display mode`);
    ...
    console.log(`  ${hasScreenshots ? '✅' : '❌'} Screenshots for enhanced install dialog`);
