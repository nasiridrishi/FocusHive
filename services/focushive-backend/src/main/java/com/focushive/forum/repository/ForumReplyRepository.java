...
public interface ForumReplyRepository extends JpaRepository<ForumReply, String> {
    ...
    List<ForumReply> findByPostId(@Param("postId") String postId);
    ...
    @Query("SELECT fr FROM ForumReply fr " +
           ...
           "AND fr.isDeleted = false " +
           "ORDER BY fr.isAccepted DESC, fr.voteScore DESC, fr.createdAt ASC")
    List<ForumReply> findTopLevelRepliesByPostId(@Param("postId") String postId);
    
    @Query("SELECT fr FROM ForumReply fr " +
           "WHERE fr.parentReply.id = :parentId " +
           "AND fr.isDeleted = false " +
           "ORDER BY fr.createdAt ASC")
    List<ForumReply> findChildReplies(@Param("parentId") String parentId);
    ...
    @Query("SELECT fr FROM ForumReply fr " +
           ...
           "AND fr.isDeleted = false")
    List<ForumReply> findAcceptedRepliesByPostId(@Param("postId") String postId);
    
    @Query("SELECT COUNT(fr) FROM ForumReply fr " +
           "WHERE fr.post.id = :postId " +
           "AND fr.isDeleted = false")
    Long countRepliesByPostId(@Param("postId") String postId);
