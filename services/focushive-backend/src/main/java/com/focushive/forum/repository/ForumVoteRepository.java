...
@Repository
public interface ForumVoteRepository extends JpaRepository<ForumVote, String> {
    ...
    @Query("SELECT SUM(fv.voteType) FROM ForumVote fv " +
           "WHERE fv.post.id = :postId")
    Integer calculatePostScore(@Param("postId") String postId);
    
    @Query("SELECT SUM(fv.voteType) FROM ForumVote fv " +
           "WHERE fv.reply.id = :replyId")
    Integer calculateReplyScore(@Param("replyId") String replyId);
