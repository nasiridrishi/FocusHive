...
-- Update existing notifications table to improve indexes and constraints
-- Add new indexes to existing notifications table for better query performance
CREATE INDEX IF NOT EXISTS idx_notifications_type_priority ON notifications(type, priority, created_at);
CREATE INDEX IF NOT EXISTS idx_notifications_read_status ON notifications(user_id, is_read, created_at);
...
COMMENT ON TABLE notifications IS 'Individual notification instances sent to users';
