...
public class BuddyPreferences {
    ...
    @ElementCollection
    @CollectionTable(name = "buddy_focus_areas", joinColumns = @JoinColumn(name = "preference_id"))
    ...
    public boolean hasFocusAreaOverlap(List<String> otherFocusAreas) {
        if (focusAreas == null || otherFocusAreas == null) return false;
        return focusAreas.stream().anyMatch(otherFocusAreas::contains);
    }
    
    public long countFocusAreaOverlap(List<String> otherFocusAreas) {
        if (focusAreas == null || otherFocusAreas == null) return 0;
        return focusAreas.stream().filter(otherFocusAreas::contains).count();
    }
}