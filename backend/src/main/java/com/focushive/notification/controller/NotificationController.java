...
public class NotificationController {
        ...
        try {
            ...
            return ResponseEntity.badRequest()
                    .body(NotificationDto.builder()
                            .build()); // This will be handled by exception handler
        }
    }
     ...
     * @param page the page number (0-based)
     * @param size the page size
     * @return paginated notifications of the specified type
     */
    @GetMapping("/type/{type}")
    @Operation(summary = "Get notifications by type", description = "Retrieves paginated notifications of a specific type for a user")
    public ResponseEntity<NotificationResponse> getNotificationsByType(
        ...
        log.debug("Getting notifications of type {} for user {} - page: {}, size: {}", type, userId, page, size);
        
        Pageable pageable = PageRequest.of(page, size);
        NotificationResponse response = notificationService.getNotificationsByType(userId, type, pageable);
        return ResponseEntity.ok(response);
    }
     ...
     *
     * @param userId the user ID
     * @param daysToKeep number of days to keep read notifications (default: 30)
     * @return count of deleted notifications
     */
    @DeleteMapping("/cleanup")
    @Operation(summary = "Clean up old notifications", description = "Deletes old read notifications for a user")
    public ResponseEntity<Map<String, Integer>> cleanupOldNotifications(
            @Parameter(description = "User ID", required = true)
            @RequestParam @NotBlank String userId,
            @Parameter(description = "Number of days to keep read notifications")
            @RequestParam(defaultValue = "30") @Min(1) int daysToKeep) {
        
        log.debug("Cleaning up old notifications for user {}, keeping {} days", userId, daysToKeep);
        
        int deletedCount = notificationService.cleanupOldNotifications(userId, daysToKeep);
